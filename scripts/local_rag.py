def quick_search(rag, query, model="gemma2:27b"):
    """Quick search mode - search, generate answer with Ollama, and offer to open PDFs"""
    ...
    for i, (doc, metadata) in enumerate(
        zip(
            results["documents"][0][:10], results["metadatas"][0][:10]
        )  # Use top 10 for context
    ):
    ...
    # Show sources
    print(
        f"📄 Found {len(results['documents'][0])} relevant chunks from {len(source_files)} PDF(s):"
    )
...
Answer:"""
    ...
    answer = rag.query_ollama(prompt, model)
    ...
    # Group by PDF for opening
    pdf_results = {}
    for doc, metadata, distance in zip(
        results["documents"][0], results["metadatas"][0], results["distances"][0]
    ):
        filename = metadata["file"]
        page = metadata["page"]  #!/usr/bin/env python3
        ...
        try:
            ...
            if not pdf_folder:
                ...
                if sample["metadatas"]:
                    ...
                    config_file = Path("./chroma_db/.pdf_folder")
                    if config_file.exists():
                        stored_folder = config_file.read_text().strip()
                        self.pdf_folder = Path(stored_folder).expanduser()
                ...
                self.pdf_folder = Path(pdf_folder).expanduser()
            ...
            self.pdf_folder = Path(pdf_folder).expanduser() if pdf_folder else None
...
def main():
    ...
    if len(sys.argv) == 3:
        ...
        if potential_path.exists() and first_arg not in known_commands:
            ...
            folder = Path(pdf_folder).expanduser()
